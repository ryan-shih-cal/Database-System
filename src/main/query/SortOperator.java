...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        records.forEachRemaining(sortedList::add);
        sortedList.sort(new RecordComparator());
        return makeRun(sortedList);
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        while (!pq.isEmpty()) {
            ...
            Integer runInt = smallestPair.getSecond();
            if (iterators.get(runInt).hasNext()) {
                Record nextRecord = iterators.get(runInt).next();
                pq.add(new Pair<>(nextRecord, runInt));
            }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (int i = 0; i < runs.size(); i += numBuffers - 1) {
            Run mergeRun = mergeSortedRuns(runs.subList(i, i + numBuffers - 1));
            listRuns.add(mergeRun);
        }
        return listRuns;
    }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            Iterator<Record> block = getBlockIterator(sourceIterator, getSchema(), numBuffers);
            Run singleRun = sortRun(block);
            sortedRuns.add(singleRun);
        }
