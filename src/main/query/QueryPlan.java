...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int minOpCost = minOp.estimateIOCost();
        List<Integer> idxCols = getEligibleIndexColumns(table);
        IndexScanOperator idxScanOp;
        SelectPredicate sP;
        int idxScanCost;
        int selectionIndex = -1;
        for (Integer idxCol : idxCols) {
            sP = selectPredicates.get(idxCol);
            idxScanOp = new IndexScanOperator(transaction, sP.tableName, sP.column, sP.operator, sP.value);
            idxScanCost = idxScanOp.estimateIOCost();
            if (idxScanCost < minOpCost) {
                minOp = idxScanOp;
                minOpCost = idxScanCost;
                selectionIndex = idxCol;
            }
        }
        minOp = addEligibleSelections(minOp, selectionIndex);
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> prevTables : prevMap.keySet()) {
            for (JoinPredicate j : joinPredicates) {
                String leftTable = j.leftTable;
                String leftCol = j.leftColumn;
                String rightTable = j.rightTable;
                String rightCol = j.rightColumn;
                QueryOperator leftTreeOp;
                QueryOperator rightTreeOp;
                QueryOperator optimalJoinOp;
                Set<String> newSet = new HashSet<>(prevTables);
                if (prevTables.contains(leftTable) && !prevTables.contains(rightTable)) {
                    leftTreeOp = prevMap.get(prevTables);
                    rightTreeOp = pass1Map.get(Collections.singleton(rightTable));
                    newSet.add(rightTable);
                } else if (prevTables.contains(rightTable) && !prevTables.contains(leftTable)) {
                    leftTreeOp = pass1Map.get(Collections.singleton(leftTable));
                    rightTreeOp = prevMap.get(prevTables);
                    newSet.add(leftTable);
                } else {
                    continue;
                }
                optimalJoinOp = minCostJoinType(leftTreeOp, rightTreeOp, leftCol, rightCol);
                result.put(newSet, optimalJoinOp);
            }
        }
        return result;
    }
    ...
    public Iterator<Record> execute() {
        ...
        for (String table : tableNames) {
            QueryOperator singleOp = minCostSingleAccess(table);
            pass1Map.put(Collections.singleton(table), singleOp);
        }
