        ...
        private Record fetchNextRecord() {
            ...
            while (true) {
                if (rightPageIterator.hasNext()) {
                    // scan right page records for matches with current left record
                    Record rightRecord = rightPageIterator.next();
                    if (compare(leftRecord, rightRecord) == 0) {
                        return leftRecord.concat(rightRecord);
                    }
                } else if (leftBlockIterator.hasNext()) {
                    // all right page records scanned for current left record
                    // move onto the next left record in block and reset right page
                    leftRecord = leftBlockIterator.next();
                    rightPageIterator.reset();
                } else if (rightSourceIterator.hasNext()) {
                    // all right page records scanned for all left records in block
                    // move onto next right page and reset the left block
                    fetchNextRightPage();
                    leftBlockIterator.reset();
                    leftRecord = leftBlockIterator.next();
                } else if (leftSourceIterator.hasNext()) {
                    // all right pages scanned for all left records in block
                    // move onto next left block and reset the right source
                    fetchNextLeftBlock();
