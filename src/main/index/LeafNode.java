...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (getKey(key).isPresent()) {
            throw new BPlusTreeException("Key already present.");
        }
        int insertIndex = InnerNode.numLessThanEqual(key, keys);
        keys.add(insertIndex, key);
        rids.add(insertIndex, rid);
        if (this.keys.size() > 2 * metadata.getOrder()) { // Case of leafNode overflow
            List<DataBox> newKeys = new ArrayList<>(keys.subList(metadata.getOrder(), keys.size()));
            List<RecordId> newRecords = new ArrayList<>(rids.subList(metadata.getOrder(), keys.size()));
            LeafNode newLeaf = new LeafNode(metadata, bufferManager, newKeys, newRecords, rightSibling, treeContext);
            List<DataBox> oldKeys = new ArrayList<>(keys.subList(0, metadata.getOrder()));
            List<RecordId> oldRecords = new ArrayList<>(rids.subList(0, metadata.getOrder()));
            keys = oldKeys;
            rids = oldRecords;
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        while(data.hasNext()) {
            // Extracting (key, rid) pair from data
            Pair<DataBox, RecordId> pair = data.next();
            DataBox key = pair.getFirst();
            RecordId rid = pair.getSecond();
            // Inserting new (key, rid) pair into tree
            keys.add(key);
            rids.add(rid);
            if (this.keys.size() > (int) Math.ceil(fillFactor * 2 * metadata.getOrder())) { // Case of leafNode overflow
                List<DataBox> newKeys = new ArrayList<>(keys.subList(keys.size() - 1, keys.size()));
                List<RecordId> newRecords = new ArrayList<>(rids.subList(rids.size() - 1, rids.size()));
                LeafNode newLeaf = new LeafNode(metadata, bufferManager, newKeys, newRecords, rightSibling, treeContext);
                List<DataBox> oldKeys = new ArrayList<>(keys.subList(0, keys.size() - 1));
                List<RecordId> oldRecords = new ArrayList<>(rids.subList(0, rids.size() - 1));
                keys = oldKeys;
                rids = oldRecords;
                rightSibling = Optional.of(newLeaf.getPage().getPageNum());
                sync();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> siblingId = Optional.of(buf.getLong());
        if (siblingId.get() == -1) {
            siblingId = Optional.empty();
        }
        int n = buf.getInt();
        for (int i = 0; i < n; ++i) {
        ...
        return new LeafNode(metadata, bufferManager, page, keys, rids, siblingId, treeContext);
    }
