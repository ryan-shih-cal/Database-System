...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        BPlusNode nextChild = getChild(childIndex);
        return nextChild.get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        // Find the correct child to insert (key, rid) pair
        int childIndex = numLessThanEqual(key, keys);
        BPlusNode nextChild = getChild(childIndex);
        Optional<Pair<DataBox, Long>> overflow = nextChild.put(key, rid);
        if (overflow.isPresent()) { // Case of child overflow
            DataBox upKey = overflow.get().getFirst();
            Long upPageNum = overflow.get().getSecond();
            int insertIndex = numLessThanEqual(upKey, keys);
            keys.add(insertIndex, upKey);
            children.add(insertIndex + 1, upPageNum);
            if (keys.size() > 2 * metadata.getOrder()) { // Case of innerNode overflow
                DataBox splitKey = keys.get(metadata.getOrder());
                List<DataBox> newKeys = new ArrayList<>(keys.subList(metadata.getOrder() + 1, keys.size()));
                List<Long> newChildren = new ArrayList<>(children.subList(metadata.getOrder() + 1, children.size()));
                ...
                List<Long> oldChildren = new ArrayList<>(children.subList(0, metadata.getOrder() + 1));
                keys = oldKeys;
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            ...
            BPlusNode nextChild = getChild(childIndex);
            Optional<Pair<DataBox, Long>> overflow = nextChild.bulkLoad(data, fillFactor);
            if (overflow.isPresent()) { // Case of child overflow
                DataBox upKey = overflow.get().getFirst();
                Long upPageNum = overflow.get().getSecond();
                keys.add(upKey);
                children.add(upPageNum);
                if (keys.size() > 2 * metadata.getOrder()) { // Case of innerNode overflow
                    DataBox splitKey = keys.get(metadata.getOrder());
                    List<DataBox> newKeys = new ArrayList<>(keys.subList(metadata.getOrder() + 1, keys.size()));
                    List<Long> newChildren = new ArrayList<>(children.subList(metadata.getOrder() + 1, children.size()));
