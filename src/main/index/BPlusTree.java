...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        LeafNode localLeaf = root.get(key);
        return localLeaf.getKey(key);
    }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        LeafNode startLeaf = root.getLeftmostLeaf();
        return new BPlusTreeIterator(startLeaf, 0);
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        int startIndex = startLeaf.getKeys().indexOf(key);
        return new BPlusTreeIterator(startLeaf, startIndex);
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> overflow = root.put(key, rid);
        if (overflow.isPresent()) { // Case of root node overflow
            List<DataBox> keys = new ArrayList<>();
            List<Long> children = new ArrayList<>();
            DataBox newRootKey = overflow.get().getFirst();
            Long oldRootPageNum = metadata.getRootPageNum();
            Long newRootPageNum = overflow.get().getSecond();
            // Creating new root node
            keys.add(newRootKey);
            children.add(oldRootPageNum);
            children.add(newRootPageNum);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(LeafNode startLeaf, int startIndex) {
            if (startLeaf.getKeys().size() == 0) {
                ridIndex = Integer.MAX_VALUE;
            }
            leaf = startLeaf;
            ridIndex = startIndex;
        }
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if (ridIndex < leaf.getRids().size()) {
                return true;
            } else {
                LeafNode currLeaf = leaf;
                while (currLeaf.getRightSibling().isPresent()) {
                    currLeaf = currLeaf.getRightSibling().get();
                    if (currLeaf.getRids().size() > 0) {
                        return true;
                    }
                }
            }
        ...
        public RecordId next() {
            ...
            if (ridIndex >= leaf.getRids().size()) {
                while (leaf.getRightSibling().isPresent()) {
                    ...
                    if (leaf.getRids().size() > 0) {
                        ridIndex = 0;
                        break;
                    }
                }
            }
            RecordId returnId = leaf.getRids().get(ridIndex);
            ridIndex += 1;
            return returnId;
        }
    }
